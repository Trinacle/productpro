<?php
/**
 * Helper functions for the ProductPro theme
 *
 * @package ProductPro
 */
...
/* ---------- Platform logos (real SVG files in assets/img/) ----
 * Returns an <img> tag pointing to the verified platform logo SVG.
...
function sdn_platform_icon( $platform, $size = 30 ) {
    ...
    $files = array(
        'shopify'     => '5f1a58272cd5b8c219db0ba4_shopify-logo.svg',
        'woocommerce' => '5f1a59d6f884854a22b65124_woocommerce-logo.svg',
        'bigcommerce' => '5f1a5a542662b9b5006821de_bigcommerce-logo.svg',
    );
    $key = strtolower( $platform );
    if ( isset( $files[ $key ] ) ) {
        return '<img src="' . esc_url( $dir . $files[ $key ] ) . '" alt="' . esc_attr( ucfirst( $key ) ) . '" style="width:' . $size . 'px;height:' . $size . 'px;object-fit:contain;" loading="lazy">';
    }
    // API fallback — inline SVG (no branded logo exists)
    if ( $key === 'api' ) {
        return '<svg viewBox="0 0 48 48" width="' . $size . '" height="' . $size . '" fill="none" stroke="currentColor" stroke-width="2"><path d="M24 4l18 10v20L24 44 6 34V14z"/><text x="24" y="29" text-anchor="middle" font-size="9" font-weight="700" fill="currentColor" font-family="Arial" stroke="none">API</text></svg>';
    }
    return '';
}
...
function sdn_real_brand_logos() {
    return array(
        // Logos will be added here as brand assets are obtained.
        // Format: array( 'name' => 'LELO', 'slug' => 'lelo', 'file' => '2024/07/lelo-300x162.png' ),
    );
}

/* ---------- Master brand directory ----------
 * Now lives in inc/brand-directory.php (286 brands from the Fantasies
 * Boutique brand spreadsheet). See sdn_brand_directory() and
 * sdn_new_brands() there.
 */
 ...
 * URLs (e.g. "https://productpro.io/wp-content/uploads/x.jpg,..."), but
...
/* ---------- Normalize a productpro.io upload URL to the current host ----------
 * The migration stored production URLs (productpro.io/wp-content/uploads/...).
 * On staging we need productpro.io/staging/5411/... and vice-versa. This
 * rewrites the host + path prefix so images resolve on whichever environment
 * we're running on.
 */
function sdn_normalize_upload_url( $url ) {
    if ( ! $url ) return $url;
    // Match any external host + /wp-content/uploads/ and rewrite to local.
...
function sdn_pagination() {
    ...
    ?>
    <nav class="sdn-pagination" aria-label="<?php esc_attr_e( 'Articles', 'productpro' ); ?>">
      <ul>
      ...
      <p class="sdn-page-info"><?php
        printf(
          esc_html__( 'Page %1$s of %2$s', 'productpro' ),
...
function sdn_cta( $title = 'Ready to take your drop shipping to the next level?', $desc = 'Request a demo today & see how ProductPro can help boost your dropshipping revenue.' ) {
    ?>
...
function sdn_platforms() {
    return array(
        'shopify' => array(
            ...
            'steps' => array(
                array( 'Install the app', 'Add ProductPro from the Shopify App Store.' ),
        ...
        'woocommerce' => array(
            ...
            'steps' => array(
                array( 'Download the plugin', 'Get the ProductPro WooCommerce plugin.' ),
        ...
        'bigcommerce' => array(
            ...
            'desc'      => 'Our industry-leading dropshipping app lets you list products from the ProductPro marketplace directly from the BigCommerce control panel. Sync inventory, automate orders, and ship blind under your brand.',
            'features'  => array(
                array( 'Native BigCommerce app', 'Install from the BigCommerce marketplace.' ),
                array( 'Direct catalog import', 'List ProductPro products from your control panel.' ),
            ...
            'steps' => array(
                array( 'Install the app', 'Add ProductPro from the BigCommerce marketplace.' ),
        ...
        'api' => array(
            ...
            'desc'      => 'Running a custom storefront or a bespoke fulfillment stack? ProductPro exposes a full REST API plus CSV/FTP and EDI-X12 options so you can integrate dropshipping into any architecture.',
            'features'  => array(
...
function sdn_integration_bubbles( $platforms = array(), $variant = '' ) {
    ...
    foreach ( $list as $key ) {
        ...
        if ( ! empty( $p['icon'] ) ) {
            echo '<span class="ib-mark">' . sdn_platform_icon( $p['icon'], 40 ) . '</span>';
        } else {
