...
if ( $sdn_requested ) {
    ...
    if ( $cpt_post && $cpt_post->post_status === 'publish' ) {
        ...
        // the migration wrote (a generic "Welcome to ProductPro..." template
        ...
        $is_boilerplate = $has_real && (
            ...
            ( stripos( $raw_content, 'Welcome to ProductPro, where innovation meets convenience' ) !== false
              && stripos( $raw_content, 'Cookies' ) !== false ) ||
            // Seeder boilerplate (created by cpt-brands.php when no migration ran).
            stripos( $raw_content, 'products are available for dropship and wholesale on ProductPro' ) !== false
        );
    ...
    <div class="wrap brand-hero-grid">
      <div class="brand-hero-left">
        <div class="brand-hero-text reveal reveal-d1">
          ...
          <p class="brand-hero-sub">Create a ProductPro account to get started.</p>
